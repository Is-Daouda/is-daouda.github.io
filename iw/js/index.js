	...
	const _dm =
		settings.difficulty === "easy"
			...
			: settings.difficulty === "hard"
				? 1.5
				: 1;
	// Niveaux 1-3 (index 0-2) : ennemis moins résistants
	const _easyLvl =
		typeof _currentLevel !== "undefined" &&
		_currentLevel <= 2;
	const _rm = _easyLvl ? 0.6 : 1;
	switch (type) {
		case "drone":
			return {
				...base,
				w: 32,
				h: 22,
				hp: Math.max(1, Math.round(2 * _dm * _rm)),
				score: 100,
		...
		case "turret":
			return {
				...
				w: 28,
				h: 28,
				hp: Math.max(1, Math.round(4 * _dm * _rm)),
				score: 200,
				speed: 55,
				color: "#445566",
				movePattern: "steady",
		...
		case "kamikaze":
			return {
				...
				w: 22,
				h: 20,
				hp: Math.max(1, Math.round(1 * _dm * _rm)),
				score: 150,
		...
		case "interceptor":
			return {
				...
				w: 28,
				h: 20,
				hp: Math.max(1, Math.round(3 * _dm * _rm)),
				score: 250,
		...
		case "carrier":
			return {
				...
				w: 55,
				h: 38,
				hp: Math.max(1, Math.round(12 * _dm * _rm)),
				_maxHp: Math.max(
					1,
					Math.round(12 * _dm * _rm),
				),
				score: 500,
				speed: 45,
				color: "#aa5500",
		...
		case "shielder":
			return {
				...
				w: 38,
				h: 30,
				hp: Math.max(1, Math.round(8 * _dm * _rm)),
				score: 400,
				speed: 50,
				color: "#0066cc",
				movePattern: "steady",
