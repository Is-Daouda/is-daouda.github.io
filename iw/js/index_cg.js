// ══════════════════════════════════════════════════════════════════
//  CRAZYGAMES SDK — intégration (chargement, cycle de jeu, pubs, bannière)
// ══════════════════════════════════════════════════════════════════
const crazySDK = (() => {
    let ready = false;
    let env = "disabled";
    ...
    function available() {
        return ready && !!CG() && env !== "disabled";
    }

    async function init() {
        if (!CG()) {
            console.warn("[CrazyGames SDK] script non chargé");
            return;
        }
        try {
            await CG().init();
            ...
            env = CG().environment;
            console.log(
                "[CrazyGames SDK] initialisé — environnement:",
                env,
            );

            // Respecter le réglage de mute global de la plateforme
            const applyPlatformSettings = (s) => {
                if (!s) return;
                try {
                    if (s.muteAudio) {
                        audio.setMusicVol(0);
            ...
            CG().game.addSettingsChangeListener(
                applyPlatformSettings,
            );
            applyPlatformSettings(CG().game.settings);
        } catch (e) {
            console.warn(
                "[CrazyGames SDK] échec de l'initialisation",
                e,
            );
        }
    }

    // Empêcher le scroll de la page (flèches / espace / molette),
    // requis lorsque le jeu est intégré dans l'iframe CrazyGames.
    ...
    // ── Pub vidéo interstitielle (entre niveaux, game over, victoire...) ──
    function requestMidgameAd(onDone) {
        ...
        try {
            CG().ad.requestAd("midgame", {
                adStarted: () => _muteForAd(),
                adFinished: () => done(),
                adError: (error) => {
                    console.warn(
                        "[CrazyGames SDK] Pub midgame indisponible",
                        error,
                    );
                    done();
                },
    ...
    // ── Pub vidéo récompensée (bonus optionnel pour le joueur) ──
        ...
        try {
            CG().ad.requestAd("rewarded", {
                ...
                adError: (error) => {
                    ...
                    _unmuteAfterAd();
                    console.warn(
                        "[CrazyGames SDK] Pub rewarded indisponible",
                        error,
                    );
                    onFail?.();
                },
    ...
    // ── Bannière statique (menu principal uniquement) ──
    let _bannerVisible = false;
    function showBanner() {
        /*if (_bannerVisible) return;
        if (!available()) return;
        const el = document.getElementById("cg-banner");
        if (!el) return;
        ...
        CG()
            ...
            .catch((e) => {
                console.warn(
                    "[CrazyGames SDK] Bannière indisponible",
                    e,
                );
    ...
    function hideBanner() {
        ...
        if (_bannerVisible && available()) {
            try {
                CG().banner.clearBanner("cg-banner");
            } catch (e) {}
        }
        _bannerVisible = false;*/
    }
...
// Initialisation la plus précoce possible : init SDK puis signal de
// début de chargement du jeu (avant même le chargement des assets IDB).
const crazySDKInit = crazySDK.init().then(() => {
    crazySDK.loadingStart();
});

// Devient true uniquement quand l'écran du menu principal est réellement
// affiché (après splash + onboarding), pour ne PAS montrer la bannière
// pendant le démarrage du jeu (splash, "tap to continue", onboarding).
let _cgMenuReady = false;
...
// ── Difficulté réduite sur les 2 premiers niveaux : ennemis moins
// résistants (PV) et qui tirent moins souvent, sans toucher aux
// vagues (composition/timing inchangés). Niveau 3+ = normal.
function _levelDifficultyMult(level) {
    if (level === 0) return 0.55;
    if (level === 1) return 0.75;
    return 1;
}
function _levelFireRateMult(level) {
    // Multiplie l'intervalle entre deux tirs (>1 = tire moins souvent)
...
function createEnemyManager() {
    const mgr = {
        ...
        update(dt, lvlData, levelTime, width, height, level) {
            // Garder _currentLevel à jour à chaque frame (utilisé pour la
            // difficulté réduite des 2 premiers niveaux : PV + fréquence de tir)
            ...
            const _dm =
                settings.difficulty === "easy"
                    ...
                    : settings.difficulty === "hard"
                        ? 1.5
                        : 1;
            // Réduction de PV supplémentaire sur les 2 premiers niveaux
            const _ldm = _levelDifficultyMult(_currentLevel);
            switch (type) {
                case "drone":
                    return {
                        ...base,
                ...
                case "carrier":
                    return {
                        ...
                        w: 55,
                        h: 38,
                        hp: Math.ceil(12 * _dm * _ldm),
                        _maxHp: Math.ceil(12 * _dm * _ldm),
                        score: 500,
                        speed: 45,
                        color: "#aa5500",
                        movePattern: "steady",
        ...
        _updateDrone(e, dt, width, height) {
            ...
            if (e.shootTimer <= 0) {
                e.shootTimer =
                    (1.8 + Math.random() * 1.5) *
                    _levelFireRateMult(_currentLevel);
...
function createBossManager() {
    const mgr = {
        ...
        spawn(bossData, width, height, level) {
            this.active = true;
            this.x = width - bossData.w / 2 - 20;
            this.y = height / 2;
            this.w = bossData.w;
            this.h = bossData.h;
            // Boss moins résistant + tire moins souvent sur les 2 premiers niveaux
            const _bldm = _levelDifficultyMult(
                level === undefined ? -1 : level,
            );
            this._levelFireMult = _levelFireRateMult(
                level === undefined ? -1 : level,
            );
            this.hp = Math.ceil(bossData.hp * _bldm);
            this.maxHp = Math.ceil(bossData.hp * _bldm);
        ...
        update(dt, width, height, bullets, particles, state) {
            ...
            if (this.shootTimer <= 0) {
                ...
                this.shootTimer =
                    Math.max(0.25, baseRate) *
                    (this._levelFireMult || 1);
...
`);
    ...
    const self = {
        ...
        _showMenu(state) {
            // Le menu principal est réellement affiché ici (après
            // splash + onboarding) — la bannière peut apparaître.
            _cgMenuReady = true;
...
Promise.all([idb.preload(), crazySDKInit]).then(() => {
    ...
    game(
        ...
        <div style="color:#555;font-size:11px;margin-top:18px;font-family:monospace;">${t("pauseHint")}</div>`;
            ...
            // ── Publicité vidéo interstitielle (CrazyGames) ─────────────────────
            // onDone est appelé une fois la pub terminée, en erreur, ou si aucune
            // pub n'est disponible — le jeu doit toujours pouvoir continuer.
            ...
            function startGame(fromSave, mode) {
                menuNav.deactivate();
                _cgMenuReady = false; // on quitte le menu — masquer la bannière
                saveGame.clear();
                {
                    const _as = achStats.get();
...
`);
            ...
            let _saveTimer = 0;
            let _cgLastGameplayActive = false;
            let _cgLastBannerWanted = false;
            let _achTimer = 0;
            let _hudTimer = 0;
            ...
            loop(function (dt) {
                // ── CrazyGames : suivi du cycle de jeu (gameplayStart/Stop) et ──
                // ── de la visibilité de la bannière, basé sur la phase courante. ──
                const _cgGameplayActive =
                    (state.phase === "playing" ||
                        state.phase === "boss" ||
                        state.phase === "transition") &&
                    !state.paused;
                if (_cgGameplayActive !== _cgLastGameplayActive) {
                    _cgLastGameplayActive = _cgGameplayActive;
                    if (_cgGameplayActive) crazySDK.gameplayStart();
                    else crazySDK.gameplayStop();
                }
                // Bannière autorisée hors gameplay : menu principal (une fois
                // réellement affiché — pas pendant le splash/onboarding),
                // game over et victoire — jamais pendant une partie ou une pause.
                const _cgBannerWanted =
                    ((state.phase === "menu" && _cgMenuReady) ||
                        state.phase === "gameover" ||
                        state.phase === "win") &&
                    !state.paused;
                if (_cgBannerWanted !== _cgLastBannerWanted) {
                    _cgLastBannerWanted = _cgBannerWanted;
                    if (_cgBannerWanted) crazySDK.showBanner();
                    else crazySDK.hideBanner();
                }
...
}); // end Promise.all([idb.preload(), crazySDKInit]).then