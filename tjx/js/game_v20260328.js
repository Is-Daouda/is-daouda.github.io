...
function generateDailyChallenges() {
	...
	const availableCategories = [...categories]; // Create a copy to track available categories
	const generatedChallenges = [];
	
	// Ensure we pick 5 different categories (cycle through if needed)
	for (let i = 0; i < 5; i++) {
		// If we've used all categories, reset the available list
		if (availableCategories.length === 0) {
			availableCategories.push(...categories);
		}
		
		// Pick a random category from available ones
		const categoryIndex = Math.floor(seededRandom(dateHash + i * 1000) * availableCategories.length);
		const category = availableCategories[categoryIndex];
		
		// Remove this category from available list to prevent duplicates
...
function spawnObstacles(params) {
	...
	if (worldOffset > 800 && spawnX - lastBirdSpawn > (1400 + Math.random() * 900) / difficultyMultiplier) {
		...
		// 20% chance to spawn in the middle of the screen, 80% chance to spawn from sides
		let birdDirection, birdX;
		if (Math.random() < 0.2) {
			// Spawn in the middle of the screen
			birdX = CANVAS_WIDTH / 2 + (Math.random() - 0.5) * 100;
			birdDirection = Math.random() > 0.5 ? 1 : -1; // Random direction
		} else {
			// Spawn from sides (original behavior)
			birdDirection = Math.random() > 0.5 ? 1 : -1; // 1 = left to right, -1 = right to left
			birdX = birdDirection > 0 ? spawnX : spawnX + 600;
		}
...
function spawnCollectibles(params) {
	...
	if (spawnX - lastEllipseSpawn > 700 + Math.random() * 500) {
		const y = 300 + Math.random() * (GROUND_Y - 500);
...
function displayAchievements() {
	...
	// Show ONLY newly unlocked achievements from this game session
	const achievementOrder = [...newlyUnlockedAchievements];
	
	// Display only newly unlocked achievements
...
function updateTutorialControls() {
	...
	if (flyDescription) {
		if (isTouchDevice()) {
			...
			flyDescription.textContent = 'Press SPACE or maintain Right-Click to fly upward with your jetpack!';
		}
	}
}
...
const translations = {
	en: {
		...
		'tutorial_fly_desc_keyboard': 'Press SPACE or maintain Right-Click to fly upward with your jetpack!',
		...
		// Launch
		'launch_instruction': 'Hold SPACE or Right-Click to charge, release to launch!',
	...
	fr: {
		...
		'tutorial_fly_desc_keyboard': 'Appuyez sur ESPACE ou maintenez le Clic Droit pour voler vers le haut avec votre jetpack!',
		...
		// Launch
		'launch_instruction': 'Appuyez sur ESPACE ou maintenez le Clic Droit pour charger, relâchez pour lancer!',
